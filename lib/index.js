...
try {
  const optionsString = _.get(process, 'env.OPTIONS') || core.getInput('options');
  if (optionsString) {
    additionalOptions = JSON.parse(optionsString);
  }
} catch (error) {
  console.error('Failed to parse OPTIONS:', error);
}

debug( 'additional options', JSON.stringify(additionalOptions, undefined, 2) )
